...
class ArticlesAylien2():

	def __init__(self):
		self.excel_file = 'pdf_moreinfo_sheet.xls'
	...
	def main(self):
		...
		print client.RateLimits()
				...
				time.sleep(30)
				print "processing %s with %s"%(filename,title)
				...
				print "Going to extract summary and Sentiment"
				...
				values = (title,summary_sentences,publishDate,json.dumps(summary_sentiment_final),json.dumps(rulebased_sentiment_final),input_type,json.dumps(aux_info),filename)
				...
				self.prog_row_count += 1
				print "Written into Excel sheet"
				print client.RateLimits()
				time.sleep(30)
